...
public class QueryBuilder {
   ...
   */
  public static <T extends AbstractEntity> Condition<T> maybe(final Object shouldApply, final Condition<T> condition) {
   ...
   * <code>null</code>em. Jeśli to napis, to nie może być pusty.
   */
  public static <T extends AbstractEntity, R> FieldCondition<T, R> maybeEq(final R value, final Function<T, R> getter) {
   ...
   * <code>null</code>em.
   */
  public static <T extends AbstractEntity> Condition<T> maybe(final boolean apply, final Condition<T> condition) {
  ...
  protected static <T extends AbstractEntity, R> FieldCondition<T, R> textOperator(
    ...
    return query -> value == null ? "1 = 1"
        : text + " " + operator + " " + query.propertyValue(value);
  }
  ...
  public static <T extends AbstractEntity, R> FieldCondition<T, R> ilike(Function<T, R> getter, R value, MatchMode matchMode) {
    return query -> textOperator("lower(" + query.resolveFieldName(getter) + ")",
        "like", matchMode.toMatchString(value.toString().toLowerCase())).print((AbstractQuery<AbstractEntity>) query);
  }
  ...
  public static <T extends AbstractEntity, R> FieldCondition<T, R> between(final Function<T, R> getter, final R min, final R max) {
    return query -> query.resolveFieldName(getter) + " BETWEEN " + query.propertyValue(min) + " AND " + query.propertyValue(max);
  }


  public static <T extends AbstractEntity, R> FieldCondition<T, R> isNull(final Function<T, R> getter) {
    return query -> query.resolveFieldName(getter) + " IS NULL";
  }


  public static <T extends AbstractEntity, R> FieldCondition<T, R> isNotNull(final Function<T, R> getter) {
  ...
  public static <T extends AbstractEntity, R> FieldCondition<T, R> not(final Condition<T> condition) {
   ...
   */
  public static <X extends AbstractEntity> FromPart leftJoin(String alias, Class<X> clazz, Condition joinCondition) {
  ...
  public static <T extends AbstractEntity, K extends AbstractEntity, R> Function<T, R> subCollection(
      Function<T, Collection<K>> getter, Function<K, R> subGetter) {
    return t -> subGetter.apply(getter.apply(t).iterator().next());
  }


  public static <T extends AbstractEntity, K extends AbstractEntity, S extends AbstractEntity, R> Function<T, R> sub(

...
public class PrintsRendererService implements IPrintsRendererService {
  ...
  public byte[] renderHtml(String templateName, Object dataModel) {
    ...
    runtimeEx(() -> freeMarkerConfiguration.getTemplate(templateName).process(dataModel, new OutputStreamWriter(baos)));
  ...
  public byte[] generatePrint(String templateName, Object dataModel) {
    return runtimeEx(() -> {
      ...
      return createPDF(renderedFile, getClass().getResource("/print/" + templateName).toURI().toString());
    });
  }
